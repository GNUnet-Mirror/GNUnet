...
tcp_plugin_get_session (void *cls, const struct GNUNET_HELLO_Address *address)
{
              ...
              &address->peer)))
  {
    ...
    GNUNET_assert(
        GNUNET_OK == GNUNET_CONTAINER_multipeermap_put (plugin->nat_wait_conns, &session->target, session, GNUNET_CONTAINER_MULTIHASHMAPOPTION_UNIQUE_ONLY));
    ...
    const struct GNUNET_MessageHeader *message)
{
  ...
  delay = plugin->env->receive (plugin->env->cls, session->address, session, message);
